  ...
  contractStatements.forEach( stmt => {
    ...
    if(svStmts.length > 0) {
      ...
      svStmts.forEach( svStmt => {
        fs.appendFileSync(`${workingDir}/coverage/classDiagram.html`, '<br />&nbsp;&nbsp;' + (svStmt.visibility === 'public' ? ' fa:fa-plus-circle ' : ' fa:fa-minus-circle ')  + ' ' +svStmt.name + ' <i>:' + svStmt.literal.literal);
    ...
    if(mStmts.length > 0) {
      fs.appendFileSync(`${workingDir}/coverage/classDiagram.html`, '<br /><br />fa:fa-pencil-square Modifier');
