...
module.exports = function instrumentSolidity(contractSource, fileName, workingDir, contractsAccessibility) {
  ...
  contractStatements.forEach( stmt => {
    ...
    if(svStmts.length > 0) {
      fs.appendFileSync(`${workingDir}/coverage/classDiagram.html`, '<br /><br />State Variables');
      svStmts.forEach( svStmt => {
        fs.appendFileSync(`${workingDir}/coverage/classDiagram.html`, '<br />&nbsp;&nbsp;' + (svStmt.visibility === 'public' ? '+' : '-')  + ' ' +svStmt.name + ' <i>:' + svStmt.literal.literal);
    ...
    if(eStmts.length > 0) {
      fs.appendFileSync(`${workingDir}/coverage/classDiagram.html`, '<br /><br />Events');
    ...
    if(mStmts.length > 0) {
      fs.appendFileSync(`${workingDir}/coverage/classDiagram.html`, '<br /><br />Modifier');
      mStmts.forEach( mStmt => {
        fs.appendFileSync(`${workingDir}/coverage/classDiagram.html`, '<br />&nbsp;&nbsp;' +mStmt.name + '()');
      })
    }

    var fdStmts = stmt.body.filter(propEquals('type', 'FunctionDeclaration'));
    if(fdStmts.length > 0) {
      fs.appendFileSync(`${workingDir}/coverage/classDiagram.html`, '<br /><br />Functions');
