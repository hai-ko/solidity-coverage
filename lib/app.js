  ...
  */
  generateCoverageEnvironment() {
    ...
    try {
      ...
      fs.writeFileSync(`${this.workingDir}/coverage/classDiagram.mmd`, "graph TD;");
      fs.writeFileSync(`${this.workingDir}/coverage/sequenceDiagram.mmd`, "sequenceDiagram");
  ...
  instrumentTarget() {
    ...
    try {
      shell.ls(`${this.coverageDir}/contracts/**/*.sol`).forEach(file => {
        if (!this.skipFiles.includes(file)) {
          ...
          const instrumentedContractInfo = getInstrumentedVersion(contract, canonicalPath, this.workingDir, this.contractsAccessibility);
    ...
    for(let contractName in this.contractsAccessibility) {
      var focusContract = this.contractsAccessibility[contractName];
      for(let accesableContractName in focusContract) {
        if(focusContract[accesableContractName] && this.contractsAccessibility[accesableContractName]) {
          if(this.contractsAccessibility[accesableContractName][contractName]) {
            
            this.contractsAccessibility[accesableContractName][contractName] = false;
            fs.appendFileSync(`${this.workingDir}/coverage/classDiagram.mmd`, '\n' + contractName + ' --- ' + accesableContractName + ';');
          } else {
            fs.appendFileSync(`${this.workingDir}/coverage/classDiagram.mmd`, '\n' + contractName + ' --> ' + accesableContractName + ';');
          }
        }
    ...
    return new Promise((resolve, reject) => {
      ...
      reader
        ...
        .on('close', () => {
          ...
          try {
            this.coverage.generate(this.events, `${this.workingDir}/contracts`, this.workingDir);
