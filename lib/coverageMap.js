...
module.exports = class CoverageMap {
  ...
  addContract(info, canonicalContractPath) {
    ...
    const fnHash = keccakhex('__FunctionCoverage' + info.contractName + '(string,uint256,address)');
   ...
   */
  generate(events, pathPrefix, workingDir) {
    ...
    for (let idx = 0; idx < events.length; idx++) {
      ...
      if (event.topics.filter(t => this.lineTopics.indexOf(t) >= 0).length > 0) {
        ...
        const data = SolidityCoder.decodeParams(['string', 'uint256', 'address'], event.data.replace('0x', ''));
        const canonicalContractPath = data[0];
        this.coverage[canonicalContractPath].f[data[1].toNumber()] += 1;

        const functionName = this.coverage[canonicalContractPath].fnMap[data[1].toNumber()].name === null ? "" : this.coverage[canonicalContractPath].fnMap[data[1].toNumber()].name;
        const contractName = this.coverage[canonicalContractPath].contractName;
        this.addressToContractMap["0x" + event.address] = contractName;
        const senderContractName = this.addressToContractMap[data[2]] ? this.addressToContractMap[data[2]] : data[2] ;

        fs.appendFileSync(`${workingDir}/coverage/sequenceDiagram.html`, '\n' + senderContractName + '->>' + contractName + ':' + functionName + '()' );
